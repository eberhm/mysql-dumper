...
let MAX_ATTEMPTS = 5;
let DEFAULT_MAX_CONCURRENT = 200;
...
function dump(config, tableName, blockName, where, destFolder, dryRun) {
    ...
    let command = 'mysqldump -u' + config.user + ' -p' + config.passwd + ' -h ' + config.host +
        ' --opt -c -e ' + config.database + ' ' + tableName + ' --where="' + where + '"'+
        ' -t --single-transaction --skip-triggers > ' + destFolder + blockName + '.' + tableName + '.data.sql';
    if (dryRun) {
        console.log(command);
